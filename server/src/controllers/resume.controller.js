...
// ─── GET /api/resumes/my ──────────────────────────────────────────────────────
// Returns the list of all resume PDFs the logged-in user has saved to Cloudinary.
...
// publicId contains slashes (e.g. "resumes/userId/file_123") so it must be
// URL-encoded by the client and decoded here.
export const deleteMyResume = async (req, res) => {
  try {
    // Decode the URL-encoded public_id (slashes are encoded as %2F)
    ...
    // Confirm this publicId actually belongs to this user before deleting
    ...
    try {
      ...
      // Log but don't block — if Cloudinary fails we still remove the DB record
